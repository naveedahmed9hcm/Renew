...
const Signupsnd = () => {
  ...
  const handleInputPassword = (event) => {
    ...
    if (newPassword.length < 8 || !/(?=.*[a-z].*[a-z])(?=.*[A-Z].*[A-Z])/.test(newPassword)) {
      setPasswordError('Password must be at least 8 characters long and contain at least 2 uppercase and 2 lowercase letters.');
    } else {
      setPasswordError('');
    }
  }
  ...
  const handleInputnumber= (event) => {
    ...
    const handlePostData = async (e) => {
      e.preventDefault();
      if (!person.trim() || !number) {
        alert('Please fill in all required fields.');
        return;
      }
      
  // if (!emailid.trim() || !/@\.com$/.test(emailid)) {
  //   alert('Please enter a valid email address with the @.com pattern.');
  //   return;
  // }

  if (!emailid.trim() || !/\.com$/.test(emailid)) {
    alert('Please enter a valid email address ending with @gmail.com or @9hcm.com.');
    return;
  }
  ...
  if (password.length < 8) {
    ...
    console.log("data", data);
       try {
        ...
        //  console.log(result.Response.organisationid, "result")
         //  sessionStorage.setItem('organizationid', result.Response.organisationid)
          console.log(result)
